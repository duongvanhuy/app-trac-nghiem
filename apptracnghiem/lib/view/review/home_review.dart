...
class HomeReview extends StatelessWidget {
  ...
  buildItem(var size, BuildContext context, value, index) {
    return InkWell(
      onTap: () {
        value.showAnswerInReview(value.listTopic[index].id);
        value.chanegColorInReview();
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => DetailReview()));
      },
      child: Container(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                  ...
                  children: [
                    Text(
                      "Bộ đề ôn tập ${value.listTopic[index].name}",
